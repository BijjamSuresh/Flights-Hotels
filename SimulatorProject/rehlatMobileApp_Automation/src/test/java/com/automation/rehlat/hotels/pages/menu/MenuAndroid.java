...
//                scrollTheScreenUpwards();
                ...
                Logger.logWarning(viewType+" :- element id is not displayed in the current active screen");
            }
        }catch (Exception exception){
...
//            HotelsScreen.checkFlightsTabIsDisplayed(); //Todo: Change the below two commented methods to hotels screen
//            HotelsScreen.tapOnMenuButton();
    ...
    public void tapOnMyTripsSubMenuButton(){
        ...
        try{
            ...
            MenuScreen.navigateToFlightsTab();
//            FlightsScreen.checkFlightsTabIsDisplayed(); //todo:- Check this line of code to hotels screen
            if (isElementEnabledById(MAIN_TAB_BAR)){
                List<WebElement> listOfTabs = driver.findElementById(MAIN_TAB_BAR).findElements(By.className(Labels_Hotels.ANDROID_TAB_BAR));
                WebElement textView = listOfTabs.get(2).findElement(By.className(Labels_Hotels.ANDROID_TEXT_VIEW));
    ...
    public void tapOnArabicInSubMenuButton(){
        ...
        try{
            if (isElementEnabledById(LANGUAGE_TITLE)){
                ...
                if (languageTitleName.equalsIgnoreCase(Labels_Hotels.ARABIC_LANGUAGE)){
                    Logger.logComment(Labels_Hotels.ARABIC_LANGUAGE+" :- sub menu element is displayed");
                    languageTitle.click();
                    Logger.logStep("Arabic option is tapped");
                }
            }else {
                Logger.logError(Labels_Hotels.ARABIC_LANGUAGE+" sub menu element is not displayed");
            }
        }catch (Exception exception){
    ...
    public void tapOnEnglishInSubMenuButton(){
        ...
        try{
            if (isElementEnabledById(LANGUAGE_TYPE)){
                ...
                String languageTitleName = languageTitle.getText();
                if (languageTitleName.equalsIgnoreCase(Labels_Hotels.ENGLISH_LANGUAGE)){
                    Logger.logComment(Labels_Hotels.ENGLISH_LANGUAGE+" :- sub menu element is displayed");
                    languageTitle.click();
                    Logger.logStep("English option is tapped");
                }
            }else {
                Logger.logError(Labels_Hotels.ENGLISH_LANGUAGE+" sub menu element is not displayed");
            }
        }catch (Exception exception){
