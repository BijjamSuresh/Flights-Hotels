...
public class PaymentOptionsAndroid extends PaymentOptionsBase {
    ...
    public static final String SELECT_YOUR_BANK_TEXT_FIELD = "Select Your Bank";
...
//            driverWait.until(ExpectedConditions.invisibilityOfElementLocated(By.id(ACTIVITY_INDICATOR)));
    ...
    public void selectingBankName(String bankName) {
        ...
        try{
            if (isElementDisplayedByAccessibilityId(SELECT_YOUR_BANK_TEXT_FIELD)){
//                Logger.logStep("KNET Payment gateway screen is displayed and moving to next step");
                driver.findElementByAccessibilityId(SELECT_YOUR_BANK_TEXT_FIELD).click();
            ...
            Logger.logError("Encountered error: Unable to check the field name xpath:- " +SELECT_YOUR_BANK_TEXT_FIELD);
        }
    }
    ...
    public void enterCardNumber() {
        ...
        try{
            ...
            if (isElementEnabledByXpath(XPATH_OF_CARD_NUMBER_VIEW)){
                ...
                String elementId = driver.findElementByXPath(xPathOfCardNumberTextField).getAttribute("resource-id");
                sendTextById(elementId,"0000000001");
                Thread.sleep(Labels_Hotels.WAIT_TIME_MIN);
...
//                pinNumberTextField.click();
                ...
                Logger.logComment(Labels_Hotels.KNET_PAYMENT_CARD_PIN_NUMBER+" :- is entered as pin number");
                Thread.sleep(Labels_Hotels.WAIT_TIME_MIN);
    ...
    public static void enterCreditOrDebitCardNumber() {
        ...
        try {
            if (isElementDisplayedById("com.app.rehlat:id/numberTextInputLayout")){
                driver.findElement(By.id("com.app.rehlat:id/numberTextInputLayout")).sendKeys(Labels_Hotels.CREDIT_OR_DEBIT_CARD_NUMBER);
                Logger.logComment("Entered the card number :- "+ Labels_Hotels.CREDIT_OR_DEBIT_CARD_NUMBER);
            }else {
    ...
    public static void enterCreditOrDebitCardPlaceHolderName() {
        ...
        try {
            if (isElementDisplayedById("com.app.rehlat:id/nameTextInputLayout")){
                driver.findElement(By.id("com.app.rehlat:id/nameTextInputLayout")).sendKeys(Labels_Hotels.CREDIT_OR_DEBIT_CARD_HOLDER_NAME);
                Logger.logComment("Entered the card expiry year :- "+ Labels_Hotels.CREDIT_OR_DEBIT_CARD_HOLDER_NAME);
                closeTheKeyboard_Android();
            }else {
                Logger.logError("Element name is not displayed in the current active screen:- ");
            }
