package com.automation.rehlat.hotels.pages.paymentsOptions;
...
public class PaymentOptionsIos extends PaymentOptionsBase {
    ...
    public void checkPaymentOptionsScreenIsDisplayed() {
        ...
        try{
            waitTillTheProgressIndicatorIsInvisibleByClassName_IOS(Labels_Hotels.IOS_ACTIVITY_INDICATOR,3);// Todo:- Three iterations of code is needed while the sold out's are happening with long timeout
            acceptTheFareDifferAlert();
            if (Labels_Hotels.CURRENT_USER_CURRENCY_TYPE.equalsIgnoreCase(Labels_Hotels.UAE_CURRENCY_TYPE) || Labels_Hotels.CURRENT_USER_CURRENCY_TYPE.equalsIgnoreCase(Labels_Hotels.SAUDI_ARABIA_CURRENCY_TYPE)){
    ...
    public void acceptTheFareDifferAlert() {
        ...
        try {
            if (isElementDisplayedByXPath(FARE_DIFFER_ALERT_XPATH)){
                ...
                waitTillTheProgressIndicatorIsInvisibleByClassName_IOS(Labels_Hotels.IOS_ACTIVITY_INDICATOR,2); // Todo:- Two iterations of code is needed while the fare differ alert is displayed
            }
            else {
...
//        String xPathOfCardNumberTextField = null;
        try{
            if (isElementEnabledByXpath(XPATH_OF_CARD_NUMBER_VIEW))
            {
                ...
                List<WebElement> listOfTextFields = cardNumberLayout.findElements(By.className(Labels_Hotels.IOS_XCUI_ELEMENT_TYPE_TEXT_FIELD));
                for (int index =0; index <= listOfTextFields.size()-1;index++){
                    String elementVisibilityStatus = listOfTextFields.get(index).getAttribute(Labels_Hotels.VISIBLE_ATTRIBUTE);
                    if (elementVisibilityStatus.equalsIgnoreCase(Labels_Hotels.STATUS_TRUE)) {
//                        index = index +1;
//                        xPathOfCardNumberTextField =  XPATH_OF_CARD_NUMBER_VIEW+"/"+Labels_Hotels.IOS_XCUI_ELEMENT_TYPE_TEXT_FIELD+"["+index+"]";
                        ...
                        listOfTextFields.get(index).sendKeys(Labels_Hotels.KNET_PAYMENT_CARD_NUMBER);
//                        driver.findElementByXPath(xPathOfCardNumberTextField).sendKeys(Labels_Hotels.KNET_PAYMENT_CARD_NUMBER);
                        Logger.logComment(Labels_Hotels.KNET_PAYMENT_CARD_NUMBER+" :- is Parsed");
                        closeTheKeyboard_iOS();
                    }else {
...
//                driver.getKeyboard().pressKey(IOSKeyCode.); // Implement keycode method to parse the values in to the card text field
                ...
                pinNumberTextField.sendKeys(Labels_Hotels.KNET_PAYMENT_CARD_PIN_NUMBER);
                Logger.logComment(Labels_Hotels.KNET_PAYMENT_CARD_PIN_NUMBER+" :- is Parsed");
                closeTheKeyboard_iOS();
            }else {
...
//            enterCreditOrDebitCardCvvNumber();
    ...
    public static void enterCreditOrDebitCardNumber() {
        ...
        try {
            if (isElementDisplayedByXPath(XPATH_OF_CREDIT_OR_DEBIT_CARD_TEXT_FIELD)){
                driver.findElement(By.xpath(XPATH_OF_CREDIT_OR_DEBIT_CARD_TEXT_FIELD)).sendKeys(Labels_Hotels.CREDIT_OR_DEBIT_CARD_NUMBER);
                Logger.logComment(Labels_Hotels.CREDIT_OR_DEBIT_CARD_NUMBER+" :- is parsed");
    ...
    public static void enterCreditOrDebitCardExpiryMonthAndYearAndCvvNumber() {
        ...
        try {
            if (isElementDisplayedByXPath(XPATH_OF_CREDIT_OR_DEBIT_CARD_EXPIRY_MONTH_AND_YEAR_TEXT_FIELD)){
                driver.findElement(By.xpath(XPATH_OF_CREDIT_OR_DEBIT_CARD_EXPIRY_MONTH_AND_YEAR_TEXT_FIELD)).sendKeys(Labels_Hotels.CREDIT_OR_DEBIT_CARD_EXPIRY_MONTH_AND_YEAR_AND_CVV_NUMBER);
                Logger.logComment(Labels_Hotels.CREDIT_OR_DEBIT_CARD_EXPIRY_MONTH_AND_YEAR_AND_CVV_NUMBER+" :- is parsed");
                Thread.sleep(Labels_Hotels.WAIT_TIME_MIN);
            }else {
    ...
    public static void enterCreditOrDebitCardCvvNumber() {
        ...
        try {
            if (isElementDisplayedByXPath(XPATH_OF_CREDITOR_DEBIT_CARD_CVV_TEXT_FIELD)){
                driver.findElement(By.xpath(XPATH_OF_CREDITOR_DEBIT_CARD_CVV_TEXT_FIELD)).sendKeys(Labels_Hotels.CREDIT_OR_DEBIT_CARD_CVV_NUMBER);
                Logger.logComment(Labels_Hotels.CREDIT_OR_DEBIT_CARD_CVV_NUMBER+" :- is parsed");
                Thread.sleep(Labels_Hotels.WAIT_TIME_MIN);
            }else {
    ...
    public static void enterCreditOrDebitCardPlaceHolderName() {
        ...
        try {
            ...
            {
                driver.findElement(By.xpath(XPATH_OF_CREDIT_OR_DEBIT_CARD_HOLDER_NAME_TEXT_FIELD)).sendKeys(Labels_Hotels.CREDIT_OR_DEBIT_CARD_HOLDER_NAME);
                Logger.logComment(Labels_Hotels.CREDIT_OR_DEBIT_CARD_HOLDER_NAME+" :- is parsed");
                closeTheKeyboard_iOS();
                Thread.sleep(Labels_Hotels.WAIT_TIME_MIN);
            }else {
    ...
    public void enterKeysInThePasswordFieldOf3DSecureCreditOrDebitCardCheckOutPayment() {
        ...
        try {
            ...
            if (isElementEnabledByXpath(XPATH_OF_PASSWORD_FIELD_IN_3D_SECURE_DEBIT_OR_CREDIT_PAYMENT_SCREEN)){
                ...
                Logger.logComment(Labels_Hotels.PASSWORD_OF_3D_SECURE_DEBIT_OR_CREDIT_PAYMENT+" :- is parsed");
                closeTheKeyboard_iOS();
                Thread.sleep(Labels_Hotels.WAIT_TIME_MIN);
