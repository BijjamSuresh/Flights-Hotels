...
//    public static final String XPATH_OF_BOOKING_COST_IN_FIRST_CELL_WITH_SIMILAR_OPTIONS_IN_SEARCH_RESULTS = "//XCUIElementTypeApplication[@name=\\\"Rehlat\\\"]/XCUIElementTypeWindow[1]/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther/XCUIElementTypeOther[1]/XCUIElementTypeTable/XCUIElementTypeCell[1]/XCUIElementTypeStaticText[4]";
    ...
    public static void waitTillFlightsSearchResultsScreenIsDisplayed() {
        ...
        try{
            if (Labels_Flights.FLIGHT_BOOKING_TYPE.equalsIgnoreCase(Labels_Flights.INTERNATIONAL_FLIGHT_BOOKING)){
                Thread.sleep(Labels_Flights.WAIT_TIME_MIN);
                if (!isElementDisplayedByName(Labels_Flights.TO_INTERNATIONAL_AIRPORT_CODE)){
                    driverWait.until(ExpectedConditions.visibilityOfElementLocated(By.name(Labels_Flights.TO_INTERNATIONAL_AIRPORT_CODE)));
                }else {
                    Thread.sleep(Labels_Flights.WAIT_TIME_MIN);
                    Logger.logStep("Active screen is loaded and moving to check the screen name");
                }
            }else if (Labels_Flights.FLIGHT_BOOKING_TYPE.equalsIgnoreCase(Labels_Flights.DOMESTIC_FLIGHT_BOOKING)){
                Thread.sleep(Labels_Flights.WAIT_TIME_MIN);
                if (!isElementDisplayedByName(Labels_Flights.TO_DOMESTIC_AIRPORT_CODE)){
                    driverWait.until(ExpectedConditions.visibilityOfElementLocated(By.name(Labels_Flights.TO_DOMESTIC_AIRPORT_CODE)));
                }else {
                    Thread.sleep(Labels_Flights.WAIT_TIME_MIN);
                    Logger.logStep("Active screen is loaded and moving to check the screen name");
                }
...
//            if (isElementDisplayedByXPath(xPathOfBookingFlightCellType)){ // Enable this line of code when the below commented logic is enabled or re implemented
                ...
                List<WebElement> flightCellDetails = bookingFlightCell.findElements(By.className(Labels_Flights.IOS_XCUI_ELEMENT_TYPE_STATIC_TEXT));
                ...
                for (int cellIndex=0;cellIndex<flightCellDetails.size();cellIndex++){
                    ...
                    String flightCellTypeText = flightCellType.getAttribute(Labels_Flights.VALUE_ATTRIBUTE);
                    if (flightCellTypeText.contains(Labels_Flights.DOT_STRING)){
                        ...
                        String bookingCostExcludingCurrencyName = flightCellTypeText.replace(Labels_Flights.CURRENT_USER_CURRENCY_TYPE+ Labels_Flights.ONE_CHARACTER_SPACE, Labels_Flights.STRING_NULL);
                        Labels_Flights.SELECTED_SEAT_BOOKING_COST = bookingCostExcludingCurrencyName;
                        Logger.logComment("Booking cost of the flight cell number - " +selectedFlightCellTypeNumber + "and with index " + cellIndex + " is :- " + Labels_Flights.SELECTED_SEAT_BOOKING_COST);
                ...
                if (Labels_Flights.SELECTED_SEAT_BOOKING_COST == null) {
                    ...
                    for (int cellIndex = 0; cellIndex < flightCellDetails.size(); cellIndex++) {
                        ...
                        if (flightCellTypeText.contains(Labels_Flights.CURRENT_USER_CURRENCY_TYPE)) {
                            ...
                            String newFlightCellTypeText = newFlightCellType.getAttribute(Labels_Flights.VALUE_ATTRIBUTE);
                            String bookingCostExcludingCurrencyName = newFlightCellTypeText.replace(Labels_Flights.CURRENT_USER_CURRENCY_TYPE, Labels_Flights.STRING_NULL);
                            Labels_Flights.SELECTED_SEAT_BOOKING_COST = bookingCostExcludingCurrencyName;
                            Logger.logComment("Booking cost of the flight cell type number - " + selectedFlightCellTypeNumber + "and with index " + cellIndex + "  is :- " + Labels_Flights.SELECTED_SEAT_BOOKING_COST);
                            bookingFlightCell.click(); // Todo: This line of code needs to be removed from this method after demo as this will do only getting the book cost
                            return Labels_Flights.SELECTED_SEAT_BOOKING_COST;
                        } else {
                            Logger.logComment("Flight cell number - " + selectedFlightCellTypeNumber + " and with index : " + cellIndex + " is not having any booking flight cost");
...
//                for (int cellIndex=0;cellIndex<flightCellDetails.size();cellIndex++){
//                    WebElement flightCellType = flightCellDetails.get(cellIndex);
//                    String flightCellTypeText = flightCellType.getAttribute(Labels_Hotels.VALUE_ATTRIBUTE);
//                    if (flightCellTypeText.contains(".")){
//                        Labels_Hotels.SELECTED_SEAT_BOOKING_COST = flightCellTypeText;
//                        Logger.logComment("Booking cost of the flight cell type number - " +selectedFlightCellTypeNumber + " is :- " + flightCellTypeText);
//                        return Labels_Hotels.SELECTED_SEAT_BOOKING_COST;
//                    }
//                    else{
//                        Logger.logComment("Flight cell number - "+selectedFlightCellTypeNumber+" and with index : " + cellIndex + " is not having any booking flight cost");
//                    }
//                }
//                // This condition is to get the flight cost when the price value is integer
//                if (Labels_Hotels.SELECTED_SEAT_BOOKING_COST == null){
//                    Logger.logAction("Checking the flight cost is an integer or not? ");
//                    for (int cellIndex=0;cellIndex<flightCellDetails.size();cellIndex++){
//                        WebElement flightCellType = flightCellDetails.get(cellIndex);
//                        String flightCellTypeText = flightCellType.getAttribute(Labels_Hotels.VALUE_ATTRIBUTE);
//                        if(flightCellTypeText.contains(Labels_Hotels.CURRENT_USER_CURRENCY_TYPE)){
//                            cellIndex = cellIndex+1;
//                            WebElement newFlightCellType = flightCellDetails.get(cellIndex);
//                            String newFlightCellTypeText = newFlightCellType.getAttribute(Labels_Hotels.VALUE_ATTRIBUTE);
//                            Labels_Hotels.SELECTED_SEAT_BOOKING_COST = newFlightCellTypeText;
//                            Logger.logComment("Booking cost of the flight cell type number - " +selectedFlightCellTypeNumber + " is :- " + Labels_Hotels.SELECTED_SEAT_BOOKING_COST);
//                            return Labels_Hotels.SELECTED_SEAT_BOOKING_COST;
//                        }
//                        else{
//                            Logger.logComment("Flight cell number - "+selectedFlightCellTypeNumber+" and with index : " + cellIndex + " is not having any booking flight cost");
...
//                valueOfBookingSeatCostDisplayedInSearchResultsScreen = driver.findElementByXPath(XPATH_OF_BOOKING_COST_IN_FIRST_CELL_WITHOUT_SIMILAR_OPTIONS_IN_SEARCH_RESULTS).getAttribute(Labels_Hotels.VALUE_ATTRIBUTE);
//                if (valueOfBookingSeatCostDisplayedInSearchResultsScreen.contains("similar option(s)")){
//                    valueOfBookingSeatCostDisplayedInSearchResultsScreen = driver.findElementByXPath(XPATH_OF_BOOKING_COST_IN_FIRST_CELL_WITH_SIMILAR_OPTIONS_IN_SEARCH_RESULTS).getAttribute(Labels_Hotels.VALUE_ATTRIBUTE);
//                    valueOfBookingSeatCostDisplayedInSearchResultsScreen = Labels_Hotels.SELECTED_SEAT_BOOKING_COST;
//                }else{
//                    valueOfBookingSeatCostDisplayedInSearchResultsScreen = Labels_Hotels.SELECTED_SEAT_BOOKING_COST;
//                }
//            }
...
//        if (Labels_Hotels.SELECTED_SEAT_BOOKING_COST.equals(null)){
