...
public class TicketBooking_WithOutCouponCode_And_WithKaramCash_BySignUp_From_MenuScreen extends BaseTest {
    @Test
    public void testTicketBooking_WithOutCouponCode_And_WithKaramCash_BySignUp_From_MenuScreen() throws Exception {
        createNewSignUpEmailId();
        Logger.beginTest("Ticket booking without coupon code and with karam cash by sign up from menu screen");
        ...
        HotelsScreen.setTheGuestCountTo(1,2,1); //Todo:- Discuss with stake holder or dev on allowing the selection of child more than adults
        HotelsScreen.tapOnAddRoomButton();
        HotelsScreen.setTheGuestCountTo(2,2,1); //Todo:- Discuss with stake holder or dev on allowing the selection of child more than adults
        ...
        if (HotelsProfileScreen.isSoldOutAlertIsDisplayed()){
            ...
            HotelsScreen.tapOnCheckInOptionInCalendarView();
            HotelsScreen.selectCheckInDate(SECOND_CHECK_IN_MONTH,"16");
            HotelsScreen.selectCheckOutDate(SECOND_CHECK_OUT_MONTH,"17");
            HotelsScreen.tapOnDoneButtonInCalendarView();
            if (HotelsProfileScreen.isSoldOutAlertIsDisplayed()){
                com.automation.rehlat.hotels.libCommon.Logger.logError("Tried two times but still the sold out alert is displaying");
            }
        }
        ...
        GuestTravellersDetailsScreen.enterTravellersDetailsForPassengers(1,2,1);
        GuestTravellersDetailsScreen.tapOnSaveButton();
        GuestTravellersDetailsScreen.enterTravellersDetailsForPassengers(2,2,1);
        GuestTravellersDetailsScreen.tapOnSaveButton();
...
//        PaymentOptionsScreen.checkTheCreditOrDebitCardBookingProcessIsSuccess();
        Logger.endTest("Sample test case 2");
    }
}
