    ...
    public static void checkBookedFlightsInformationIsDisplayed(String parsingFlightType){
        ...
        try {
            if (isElementDisplayedByXPath(XPATH_OF_FLIGHTS_TRAVELLING_DETAILS_TABLE)){
                WebElement xcuiElementTypeCell = driver.findElementByXPath(XPATH_OF_FLIGHTS_TRAVELLING_DETAILS_TABLE).findElement(By.className(Labels_Flights.IOS_XCUI_ELEMENT_TYPE_CELL));
                WebElement xcuiElementStaticText = xcuiElementTypeCell.findElement(By.className(Labels_Flights.IOS_XCUI_ELEMENT_TYPE_STATIC_TEXT));
                String elementNameInTheCell = xcuiElementStaticText.getAttribute(Labels_Flights.VALUE_ATTRIBUTE);
